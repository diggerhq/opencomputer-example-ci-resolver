...
type Failure = { repository?: string; ref?: string; job?: string; log?: string };
...
function resolvePrompt({ repository, ref, job, log }: Failure, text?: string) {
  return `\
...
1. git clone https://github.com/${repository} repo, then git checkout ${ref ?? "HEAD"}. Run the failing test named in the log and confirm it fails.
